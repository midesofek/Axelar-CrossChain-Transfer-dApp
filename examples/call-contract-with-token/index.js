...
async function deploy(chain, wallet) {
  ...
  console.log(
    `Deployed DistributionExecutable for ${chain.name} at ${chain.distributionExecutable}.`
  );
}
...
async function test(chains, wallet, options) {
  ...
  async function logAccountBalances() {
    for (const account of accounts) {
      console.log(
        `${account} has ${(await usdcContract.balanceOf(account)) / 1e6} aUSDC`
      );
    }
  }
